          ...
          then [
            {
              ...
              def = v.defaultText or (if v ? default then builtins.toJSON v.default else "");
            }
          ]
          else if builtins.isAttrs v then toList v cur else []
      ) optSet
    );
in rec {
  ...
  getFeatureOptionsItems = featuresModule: let
    eval = lib.evalModules {
      modules = [
        ...
        featuresModule
        ({ lib, ... }: {
          options.assertions = lib.mkOption { type = lib.types.anything; visible = false; description = "internal"; };
        })
      ];
      check = false;
    };
    opts = eval.options;
    items = toList opts "";
  in lib.filter (o: !(lib.hasPrefix "assertions" o.path)) items;

  renderFeaturesOptionsMd = items:
    let
  ...
  renderDeltasMd = { flatNeg, flatLite }:
    let
      keys = lib.unique ((builtins.attrNames flatNeg) ++ (builtins.attrNames flatLite));
      rows = lib.concatStringsSep "\n" (
        map (
          k:
            let a = flatNeg.${k} or null; b = flatLite.${k} or null; in
            if a != b then "| ${k} | ${toString a} | ${toString b} |" else ""
        ) keys
      );
