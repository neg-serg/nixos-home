...
- Профили и фичи
  ...
  - Редакторы: `features.dev.emacs.enable` собирает кастомный `emacs30-pgtk` (emacs-overlay,
    доп. tree-sitter/QML-грамматики) и запускает `services.emacs`.
  - Аудио: `features.media.audio.core/apps/creation/mpd.enable` — ставит TUI‑клиент `rmpc`, Qt‑клиент
    `pkgs.neg.cantata`, экспортирует `MPD_HOST`/`MPD_PORT` и при желании автозапускает Cantata
    (`media.audio.mpd.cantata.autostart = true;`).
  - Трейдинг: `features.finance.tws.enable` ставит Trader Workstation от Interactive Brokers (по умолчанию выключено).
