...
with lib;
  mkIf config.features.gui.enable (lib.mkMerge [
    (let
      hy3Perms = if (config.features.gui.hy3.enable or false) then ''
      ...
      '' else "";
      hyprsplitPerms = if (config.features.gui.hyprsplit.enable or false) then ''
        # Allow loading hyprsplit plugin
        permission = ^/nix/store/[^/]+-hyprsplit-[^/]+/lib/[^/]*hyprsplit[^/]*\.so$, plugin, allow
      '' else "";
      hyprVrrPerms = if (config.features.gui.vrr.enable or false) then ''
    ...
    in
      ...
      '' + hy3Perms + hyprsplitPerms + hyprVrrPerms)
    )
