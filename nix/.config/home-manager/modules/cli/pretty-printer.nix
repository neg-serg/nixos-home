...
lib.mkMerge [
  ...
  }
  {
    ...
    home.sessionVariables.PYTHONPATH = let
      ...
      libneovim = if pkgs.python3Packages ? neovim then "${pkgs.python3Packages.neovim}/${pkgs.python3.sitePackages}" else "";
      extra = builtins.filter (x: x != "" && x != null) [ libdocopt libannoy liborjson libnumpy libneovim ];
    in lib.concatStringsSep ":" ([ libpp libcolored ] ++ extra);
  }
  {
