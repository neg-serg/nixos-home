...
- Spacing between the VPN and ethernet icons now keys entirely off `network.capsule.iconPadding`: we subtract half of the *absolute* value from the leading slot spacing/margins, so increasing the magnitude tightens the pair (set positive or negative numbers to taste). The default theme uses `-8` for an aggressively compact layout.
...
- Чтобы иконки VPN и Ethernet стояли ближе, мы вычитаем половину абсолютного значения `network.capsule.iconPadding` из spacing и внешнего отступа leading-слота. Чем больше модуль значения (положительный или отрицательный), тем сильнее сжатие; по умолчанию стоит `-8` для очень плотной посадки.
