...
- Spacing between the VPN and ethernet icons now keys entirely off `network.capsule.iconPadding`: we subtract half of that token from the leading slot spacing/margins, so raising the value loosens both slots equally. The default theme pins the token to `0` for the tightest layout.
...
- Чтобы иконки VPN и Ethernet стояли ближе, мы вычитаем половину `network.capsule.iconPadding` из spacing и внешнего отступа leading-слота. Один и тот же токен контролирует обе иконки, а в дефолтной теме он равен `0`, так что пара сидит максимально плотно.
